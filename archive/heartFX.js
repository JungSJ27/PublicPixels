...
function injectHearts(){
      ...
      <svg viewBox="0 0 24 24" class="heart-icon" aria-hidden="true">
        <defs>
          ...
          <radialGradient id="heartShade" cx="50%" cy="45%" r="72%">
            ...
            <stop offset="40%"  stop-color="rgba(0,0,0,0.11)"/>
  ...
  /** 버튼에 3D 하트를 보장하고, 로드시 3바퀴 버스트 */
  function ensure3D(btn, card, id){
    if (btn._heart3D) return; // 이미 있음
    // 캔버스는 아이콘 박스와 픽셀-정렬
  ...
  Heart3D.loadModel(model=>{
    const h = new Heart3D(canvas, model);
    btn._heart3D = h;
    
    startTwinkleFor(btn);

    // ✅ 정면 기준 고정 (원하면 0 대신 살짝 튠)
    h.model.rotation.y = 0;  // 현재 모델을 '정면'으로 맞춰놓기
    h.frontYaw = 33;          // 정면 각도(라디안). 예: -8도 => -(Math.PI/180)*8

    h.start();
    h.spinBurst(3, 0.9, h.frontYaw); // ✅ 정면에서 멈추게
    if (Math.random() < 0.33) setTimeout(()=> sparkle(btn), 240);
    btn.classList.add('is-on');
  });
  }
  ...
  /* =================== FX: smoke =================== */
  function smoke(button, aOrOpts, b){
    //smoke(btn, { mode:'dark', w:60, h:120, boost:1.4, yOffset:6 })
    const opts = typeof aOrOpts === 'object' && aOrOpts
      ? aOrOpts
      : { a: aOrOpts, b };

    const icon = button.querySelector('.heart-icon') || button;
    const r = icon.getBoundingClientRect();
    const vx = r.left + r.width * 0.5;
    const vy = r.top  + r.height * -0.3+ (opts.yOffset || 0);

    const el = document.createElement('div');
    el.className = 'heart-smoke';

    // 크기 조절
    if (opts.w) el.style.setProperty('--smoke-w', typeof opts.w === 'number' ? `${opts.w}px` : opts.w);
    if (opts.h) el.style.setProperty('--smoke-h', typeof opts.h === 'number' ? `${opts.h}px` : opts.h);

    // 색상 및 강도 조절
    const a = opts.a || 'rgba(0,0,0,.60)';  // 바깥
    const b2 = opts.b || 'rgba(0,0,0,.38)'; // 안쪽
    el.style.setProperty('--heart-smoke-a', a);
    el.style.setProperty('--heart-smoke-b', b2);

    if (opts.boost) el.style.setProperty('--smoke-boost-k', String(opts.boost));
    ...
    const sc  = 0.84 + Math.random()*0.14;
