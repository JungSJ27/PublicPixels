...
export class Heart3D {
  ...
  static loadModel(callback){
    ...
    loader.load('secass3/Sec3Heart.glb', (gltf)=>{
      const model = gltf.scene;

      // 기본 유리 재질  핑크
    const pinkMaterial = new MeshPhysicalMaterial({
      color: 0xff4dbd,            // 표면 틴트  더 핑크
      transmission: 0.06,         // 투명도 낮게  덜 유리 느낌
      thickness: 0.45,            // 두께
      ior: 1.5,                   // 굴절률
      roughness: 0.32,            // 살짝 매트
      metalness: 0.0,
      attenuationColor: 0xff2a88, // 내부 색  핫핑크 코어
      attenuationDistance: 0.9,
      side: DoubleSide,
      transparent: false,
      opacity: 1
    });
    model.traverse(n => { if (n.isMesh) n.material = pinkMaterial; });
